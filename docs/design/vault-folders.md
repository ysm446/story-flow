...
- **後方互換**: 既存カードは全部ルート（folder_id NULL）なので、フォルダを
  ...
  **ルートのみ**。
  - 2026-07-11 修正: 当初はフロントが未選択時に folder_ids を送らず、在庫 API 側の
    「未指定 = 全カード」互換挙動に落ちていたため、チェックなしでもおまかせが
    全フォルダから引いてしまうバグがあった（アセット一覧の見え方と不一致）。
    現在はフロントが**空配列を必ず送り**、backend は「空リスト = ルートのみ /
    None = 全カード（API 直叩き互換）」と解釈を分ける。
