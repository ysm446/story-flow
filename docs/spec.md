...
- **テイクモデル（2026-07-09 追加）**: 生成（全体・部分とも）は既存 story を上書きせず、
  常に新しい story（テイク）として保存する。`parent_story_id` で部分再生成の系譜を持つ。
  後戻り = 過去のテイクを選ぶだけで、アンドゥ機構は持たない。
- **部分再生成**: シーン k の撮り直しは、テイクの `story_scenes[k-1].state_after` から
  StoryState を復元してパイプラインを再開する（k 未満のシーンはコピー）。
  mode=single（1 シーンのみ）では以降のシーンもコピーされるが、確定事実が
  ずれる可能性があるため UI で「要確認」を表示する。
...
```
GET    /stories               # 履歴一覧（?workspace_id= で作品絞り込み）
GET    /stories/{id}          # 再生用フル取得（scenes 順）
DELETE /stories/{id}
```

### 8.4 その他（実装済みの追加ルート。詳細は各ファイル）

```
routes/workspaces.py   # 作品（§4.7）の CRUD / 複製
routes/bgm.py          # BGM ライブラリ（CRUD / 音源アップロード・配信 / FTS・ベクトル検索）
routes/prompts.py      # プロンプトプリセット（一覧 / 追加 / 編集 / 削除 / アクティブ切替）
routes/library.py      # ライブラリの状態取得 / 新規作成・切り替え（§3 ライブラリ = 作品バンドル）
```
...
story-flow/
  ...
  backend/
    main.py
    db/
      schema.sql             # 追加カラムは database.py の条件付き ALTER で移行
      database.py
    routes/
      cards.py
      generate.py
      stories.py
      workspaces.py          # 作品（§4.7）
      bgm.py                 # BGM ライブラリ
      prompts.py             # プロンプトプリセット
      library.py             # ライブラリの新規作成/切り替え
    services/
      embedding.py           # Qwen3-Embedding llama-server の HTTP ラッパー（/v1/embeddings）
      ...
      media.py               # メディア保存（sha256 命名）+ サムネイル
      prompts.py             # プリセットの読み書き（ライブラリの prompts.json）
      bgm_select.py          # BGM 自動選曲（候補検索 → LLM 選択）
    prompts/
      writer.md
      selector.md
  data/
