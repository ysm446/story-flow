...
更新日時: 2026-07-10 18:27

## 現在地

**v1.5 の中核 fill_gap 実装済み + 作者の実 LLM 通し確認 OK（2026-07-10「現状で上手く
いっています」）**。Vault のフォルダ整理と作品ごとの「使うフォルダ」も実装。
残りは多様性チューニング・浅いバックトラック（spec §7）と、フォルダ機能のアプリ確認。
...
- 2026-07-10: Compose キャンバスのショートカット（作者要望）— **A = 全体表示（fitView）/
  ...
  キャンバス内にフォーカスがあるときのみ有効（keydown はキャンバスのラッパー div で拾い、
  input/textarea/select 内や修飾キー押下時は無視）。検証: build 成功。操作感はアプリ確認待ち
...
1. アプリで**フォルダ機能の確認**: Vault でフォルダ作成 → カードをドラッグで整理 →
   フォルダ同士の並べ替え・入れ子化 → Compose「使うフォルダ」でアセットとおまかせの
   絞り込みが効くか
