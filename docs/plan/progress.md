...
**フェーズ 3（Theater）完了**。生成 → 鑑賞まで一本つながった。
残りはフェーズ 4（Compose: React Flow アンカー配置）で v1 完成。
...
フェーズ 4（Compose）で v1 完成: React Flow（`@xyflow/react` 導入済み）で
始点・中間（複数可）・終点ノードを置いて線で繋ぐ最小ビュー。エッジの意味は
「並び順」のみ（分岐の意味論は v2 まで持ち込まない）。lm-graph の
ReactFlowProvider / useNodesState / カスタムノード / RoundedSmoothStepEdge を下敷きに。
完成後、Generate 画面の暫定アンカー選択 UI を composition ドラフト
（store/appStore の CompositionDraft）経由に置き換える。
