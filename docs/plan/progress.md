...
- 2026-07-12: **Theater 本文の表示高さスライダー**（作者要望）— UI 設定
  theaterTextHeightPct（既定 25%・10〜60%）を追加。単位は「ステージの高さに対する %」
  （従来の 25vh = ウィンドウ基準から変更。額縁・全画面でも割合が保たれる）。
  検証: build 成功。見た目はアプリ確認待ち
