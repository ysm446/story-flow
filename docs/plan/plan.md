...
### 項目: 分岐ノード（選択肢。2026-07-11 作者アイデア。v2 筆頭・未実装）

- 設計資料: [docs/design/branch-nodes.md](../design/branch-nodes.md)
- Compose で出力エッジを複数許し、各エッジ = 選択肢。Theater が分岐点で選択肢を出して
  プレイヤーが枝を選ぶ（ADV 風）。spec「分岐の意味論は v2」の具体化。
- 実現性は中〜大: state_after スナップショットが分岐の土台としてそのまま使える。
  難所は story_scenes の木構造化（スキーマ変更）と Theater の選択肢再生。
- 最小形の制約: **木のみ・合流なし**（StoryState の枝間矛盾を避ける）、
  選択肢テキストはエッジのプロパティで作者が書く（LLM 生成は後から）。
- v1.5 の残り（多様性チューニング・時系列の先読み）が落ち着いてから着手。
