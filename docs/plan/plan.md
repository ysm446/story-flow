...
### 将来項目: BGM（2026-07-09 作者要望。v2 の「TTS + BGM」の具体化）

- **mp3 を登録**できる BGM ライブラリを作り、**ムードに合わせて再生**する演出を入れる。
- 実装の方向（着手時に確定）:
  - 保存はメディアと同様にライブラリルート配下（`data/library/bgm/` + sha256 命名、
    DB にはパスのみ）。曲に mood タグ（card_tags の mood 語彙と揃える）を付ける。
  - 再生時のムード判定は、シーンの `state_after.tone_so_far` か カードの mood タグを使い、
    Theater でシーン切替時に該当 BGM へクロスフェードする。
  - 音量・オン/オフは設定画面（`SettingsPanel`）に置く。
